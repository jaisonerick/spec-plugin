...
Requires the [`plaud` CLI](https://github.com/jaisonerick/plaud-cli), authenticated, and Python 3 (standard library only). Both are local, so this does not run in a cloud sandbox.
...
The hub directory then holds `catalog.jsonl` (source of truth, git-tracked, one JSON object per recording), a git-ignored sqlite index rebuilt from it, and the transcripts pulled so far. Each recording carries a `status` — `pending`, `transcribed`, `filed`, `excluded` — which is what keeps the same call from being processed twice. Curation survives every refresh; only the Plaud-side fields are overwritten.
